...
export class TreeModel<T, TFilterData = void> {
	...
	private _updateNodeAfterFilterChange(node: IMutableTreeNode<T, TFilterData>, result: ITreeNode<T, TFilterData>[], revealed = true, first = true): boolean {
		if (!first) {
			this.updateNodeFilterState(node);
		}
		...
		const revealedChildrenCountBaseline = result.length;
		node.revealedCount = 1;

		let hasVisibleDescendants = false;
		if (typeof node.visible === 'undefined' || !node.collapsed) {
			for (const child of node.children) {
				hasVisibleDescendants = this._updateNodeAfterFilterChange(child, result, revealed && !node.collapsed, false) || hasVisibleDescendants;
			}
		}

		const revealedChildrenCount = result.length - revealedChildrenCountBaseline;
		...
		if (!node.visible) {
			...
			node.revealedCount += revealedChildrenCount;
		}
